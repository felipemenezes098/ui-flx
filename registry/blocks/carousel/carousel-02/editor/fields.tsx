...
import { values as defaults } from '../carousel-02-example'

import type { Carousel02Props } from '../carousel-02'

interface Carousel02EditorFieldsProps {
  props?: Carousel02Props
  onUpdate?: (props: Carousel02Props) => void
}

export function Carousel02EditorFields({
  props: externalProps,
  onUpdate,
}: Carousel02EditorFieldsProps = {}) {
  const [internalProps, setInternalProps] =
    React.useState<Carousel02Props>(defaults)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between space-x-2">
        ...
        <Switch
          id="showNavigation"
          checked={props.showNavigation}
          onCheckedChange={(checked) => updateField('showNavigation', checked)}
        />
      </div>
        ...
        {props.items.map((item, index) => (
          ...
          >
            ...
            <div className="flex items-center justify-between rounded-md border p-3">
              <Label className="text-sm font-medium">Image Overlay</Label>
              <Switch
                checked={item.media.overlay ?? true}
