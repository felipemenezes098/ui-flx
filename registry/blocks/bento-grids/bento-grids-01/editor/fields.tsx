...
import { values as defaults } from '../bento-grids-01-example'

import type { BentoGrids01Props } from '../bento-grids-01'

interface BentoGrids01EditorFieldsProps {
  props?: BentoGrids01Props
  onUpdate?: (props: BentoGrids01Props) => void
}

export function BentoGrids01EditorFields({
  props: externalProps,
  onUpdate,
}: BentoGrids01EditorFieldsProps = {}) {
  const [internalProps, setInternalProps] =
    React.useState<BentoGrids01Props>(defaults)

  const props = externalProps ?? internalProps

  const commit = (next: BentoGrids01Props) => {
    if (onUpdate) onUpdate(next)
    else setInternalProps(next)
  }

  const updatePrimary = (patch: Partial<BentoGrids01Props['primary']>) => {
    commit({
      ...props,
  ...
  const updateItems = (items: BentoGrids01Props['items']) => {
    commit({ ...props, items })
  }
  ...
  const updateItem = (
    index: number,
    patch: Partial<BentoGrids01Props['items'][number]>,
  ) => {
