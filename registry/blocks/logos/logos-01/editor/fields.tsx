...
import { values as defaults } from '../logos-01-example'

import type { Logos01Props } from '../logos-01'

interface Logos01EditorFieldsProps {
  props?: Logos01Props
  onUpdate?: (props: Logos01Props) => void
}

export function Logos01EditorFields({
  props: externalProps,
  onUpdate,
}: Logos01EditorFieldsProps = {}) {
  const [internalProps, setInternalProps] =
    React.useState<Logos01Props>(defaults)

  const props = externalProps ?? internalProps

  const updateItems = (items: Logos01Props['items']) => {
