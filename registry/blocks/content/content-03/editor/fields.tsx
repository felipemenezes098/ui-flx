...
import { values as defaults } from '../content-03-example'

import type { Content03Item, Content03Props } from '../content-03'

interface Content03EditorFieldsProps {
  props?: Content03Props
  onUpdate?: (props: Content03Props) => void
}

function createEmptyItem(index: number): Content03Item {
  return {
    label: `Tab ${index + 1}`,
...
export function Content03EditorFields({
  props: externalProps,
  onUpdate,
}: Content03EditorFieldsProps = {}) {
  const [internalProps, setInternalProps] =
    React.useState<Content03Props>(defaults)

  const props = externalProps ?? internalProps
  const items = props.items ?? []

  const update = (newProps: Content03Props) => {
    if (onUpdate) {
      onUpdate(newProps)
    } else {
