...
import { values as defaults } from '../content-05-example'

import type { Content05Item, Content05Props } from '../content-05'

interface Content05EditorFieldsProps {
  props?: Content05Props
  onUpdate?: (props: Content05Props) => void
}

export function Content05EditorFields({
  props: externalProps,
  onUpdate,
}: Content05EditorFieldsProps = {}) {
  const [internalProps, setInternalProps] =
    React.useState<Content05Props>(defaults)

  const props = externalProps ?? internalProps

  const update = (partial: Partial<Content05Props>) => {
    const newProps: Content05Props = { ...props, ...partial }
    if (onUpdate) {
      onUpdate(newProps)
    } else {
  ...
  const updateItem = (index: number, partial: Partial<Content05Item>) => {
