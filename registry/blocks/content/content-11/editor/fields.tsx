...
import type { Content11Props } from '../content-11'

interface Content11EditorFieldsProps {
...
export function Content11EditorFields({
  props: externalProps,
  onUpdate,
}: Content11EditorFieldsProps = {}) {
  const [internalProps, setInternalProps] =
    React.useState<Content11Props>(defaults)

  const props = externalProps ?? internalProps

  const updateMedia = (field: 'src' | 'alt', value: string) => {
    const newProps: Content11Props = {
      ...props,
  ...
  const updateItem = (index: number, field: string, value: string) => {
    ...
    const newProps: Content11Props = { ...props, items: newItems }
