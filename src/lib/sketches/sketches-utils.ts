...
export function buildSketchPrompt(
  ...
  const previewUrl = `${siteConfig.url}/sketches#${item.name}`
  const source = formatCodeFilesForPrompt(codeFiles)

  const sections = [
    `# ${item.title}`,
    `## Context\nFlexnative UI sketch — a low-fidelity, content-free wireframe of a full screen from the shadcn/ui registry (@${siteConfig.codeName}).`,
