...
export function buildBlockPrompt(
  ...
  const previewUrl = `${siteConfig.url}/blocks/${categorySlug}/${item.name}`
  const source = formatCodeFilesForPrompt(codeFiles)

  const sections = [
    `# ${item.title}`,
    `## Context\nFlexnative UI block — a reusable, composable section reference implementation from the shadcn/ui registry (@${siteConfig.codeName}).`,
