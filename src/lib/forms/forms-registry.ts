...
/**
 * Slug -> form component, lazily imported. Lives in lib so any surface
 * (gallery, library page) can resolve a form without reaching into a
    ...
    formsLibraries.flatMap((lib) =>
      lib.categories.flatMap((cat) =>
        cat.items.map((item) => [
          item.slug,
          dynamic(() =>
            import(`registry/forms/${lib.slug}/${cat.slug}/${item.slug}`).then(
              (m) => ({
                default:
                  m.default ??
